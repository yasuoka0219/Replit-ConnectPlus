...
def init_db():
    ...
    with app.app_context():
        ...
        # マスターデータ（企業規模・顧客ステータス・リードソース）の初期化
        try:
            ...
            db.session.commit()
            print("✓ マスターデータ（企業規模・顧客ステータス・リードソース）を確認しました")
        except Exception as e:
            print(f"⚠️ マスターデータの初期化中にエラー: {e}")
            db.session.rollback()
...
def create_deal():
    if request.method == 'POST':
        ...
        # Parse lead_source_id
        lead_source_id = request.form.get('lead_source_id')
        lead_source_id = int(lead_source_id) if lead_source_id and lead_source_id != '' else None
        ...
        if first_contact_date_str:
            try:
                first_contact_date = datetime.strptime(first_contact_date_str, '%Y-%m-%d').date()
            except ValueError:
                pass
        ...
        deal = Deal(
            ...
            new_or_existing=request.form.get('new_or_existing') or None,
            gross_profit=float(request.form.get('gross_profit', 0)) if request.form.get('gross_profit') else None,
            probability_rank=request.form.get('probability_rank') or None,
            competitor_name=request.form.get('competitor_name') or None
        )
...
@login_required
def update_account():
    """Update user account information (name and email)"""
    try:
        ...
        log_security_event('account_updated', f'User {user.email} updated account information', user.id, ip_address=get_client_ip(), user_agent=get_user_agent())
        
        return jsonify({'success': True, 'message': 'アカウント情報を更新しました'})
    except Exception as e:
        db.session.rollback()
        import traceback
        app.logger.error(f"[Account Update] エラーが発生しました: {str(e)}\n{traceback.format_exc()}")
        return jsonify({'success': False, 'error': 'アカウント情報の更新中にエラーが発生しました'}), 500
